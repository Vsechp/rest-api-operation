...
import { getOperations, removeOperation, forceRemoveOperation } from '../../src/actions/operations';
...
const OperationCard: React.FC<OperationCardProps> = ({ onDelete }) => {
  ...
  return (
    <div>
      ...
      <Button variant="contained" color="primary" onClick={() => setIsEditOperationOpen(true)}>
        Edit Operation
      </Button>
      <Button variant="contained" color="secondary" onClick={() => handleOpenDeleteDialog(false)}>
        Delete Operation
      </Button>
      <Button variant="contained" onClick={() => handleOpenDeleteDialog(true)}>
  Soft Delete Operation
      ...
      <List>
        {suboperations.map((sub) => (
          <ListItem key={sub.id}>
            ...
            <Button variant="contained" color="primary" onClick={() => handleEditSuboperation(sub)}>
              Edit
            </Button>
      ...
      <Button variant="contained" color="primary" onClick={() => setIsAddSuboperationOpen(true)}>
        Add Suboperation
      </Button>
      {isEditOperationOpen && <EditOperationDialog operation={operation} onClose={() => setIsEditOperationOpen(false)} />}
      {isAddSuboperationOpen && <SuboperationFormWrapper operationId={operation.id} onClose={() => setIsAddSuboperationOpen(false)} />}
      {editingSuboperation && <EditSuboperationDialog suboperation={editingSuboperation} onClose={() => setEditingSuboperation(null)} />}
      {isDeleteDialogOpen && <OperationDeleteDialog isOpen={isDeleteDialogOpen} onClose={handleCloseDeleteDialog} onConfirm={handleDeleteOperation} operationId={id || ''} softDelete={softDelete} />}
    </div>
  );
};
